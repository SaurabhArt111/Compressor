...
npm run dev     # starts the backend (:5555) and the Vite dev server (:5173)
```
...
npm start       # serves the API + the built frontend from :5555
```

Open **http://localhost:5555**. Requires Node.js **18.17+** (20+ recommended).
...
Open **http://localhost:5555**. Uploaded/compressed files and history persist
...
|---|---|---|
| `PORT` / `HOST` | `5555` / `0.0.0.0` | Where the server listens |
...
user), exposes `5555`, and persists `server/uploads` and `server/data` in
...
npm install
npm run build
NODE_ENV=production PORT=5555 npm start
```
